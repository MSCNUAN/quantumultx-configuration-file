# 🌊 暖暖Quantumult X 懒人配置 (国内免流特供版)

![GitHub stars](https://img.shields.io/github/stars/MSCNUAN/QuantumultX?style=flat-square&color=ff69b4)
![GitHub last commit](https://img.shields.io/github/last-commit/MSCNUAN/QuantumultX?style=flat-square&color=orange)
![Author](https://img.shields.io/badge/Author-NuanNuan-blue?style=flat-square)

> ⚠️ **写在前面**：
> * 本配置专为 **小白用户** 打造，主打“懒人免流”与“纯净体验”。
> * 配置修改自大佬 **墨鱼 (@ddgksf2013)** 的作品，感谢开源精神！
> * 🧪 **特别说明**：由于本人目前没有免流卡，**无法亲自测试具体的跳点情况**，策略纯属根据以往经验修改。
> * 🔄 **关于更新**：本项目随缘更新，仅供参考，非常欢迎各位大佬复刻或提交 Issue 进行二改优化！

## 📂 文件说明

| 文件名 | 说明 |
| :--- | :--- |
| `暖暖自用懒人配置·国内免流版.conf` | 集成国内免流策略、去开屏广告、VIP 解锁、Boxjs 订阅等功能。 |

## 🌟 核心特色

1.  **📶 独家国内免流**：预设 `国内流量` 策略组，通过正则自动筛选国内/免流节点（需搭配含免流节点的机场）。
2.  **🚫 全方位净化**：强力去除主流 App（微博、知乎、开屏广告等）干扰。
3.  **⚡️ 自动优选**：每 15 分钟自动检测并连接最快节点，拒绝手动测速。
4.  **🛡 贴心优化**：内置 Boxjs 订阅接口，默认拦截 iOS 系统更新。

## 🛠 使用说明

1.  **核心原理**：
    * 利用 Quantumult X 的策略分流功能，将国内流量指向指定的免流节点，实现定向免流。
2.  **必备条件**：
    * **必须搭配国内节点使用！** (这是免流生效的关键)。
    * 你的机场需要支持免流（通常节点名会包含“免流”字样以及端口是80或443），需要拥有免流卡对应的混淆参数。
3.  **导入方式**：
    * 点击上方文件名 `暖暖自用懒人配置·国内免流版.conf` -> 点击 `Raw` -> 复制链接。
    * 在 Quantumult X 右下角风车 -> 配置文件 -> 下载。
4.  **开启免流**：
    * 导入后在 QX 首页点击圆球图标 -> 找到 **「国内流量」** 策略组 -> 选中你的免流节点。

---

## 🧩 进阶玩法 (自定义规则)

觉得目前的规则不够用？想要添加更多的 **分流策略** 或者 **去广告规则**？
我专门整理了一份详细教程，手把手教你如何扩展配置：

👉 **[点击查看：Quantumult X 添加分流与去广告保姆级教程](http://www.nuan1145.eu.cc/archives/wei-ming-ming-wen-zhang-ukUoeGpM)**

---

## 📢 关注我 & 实用资源推荐

这里汇总了我的联系方式以及我亲测好用的工具与服务，欢迎自取！

### ✈️ 关注我的 Telegram

* **频道 (资源发布/动态)**：【👉 [点击订阅](https://t.me/NUAN114514)】
* **群组 (吹水/交流)**：【💬 [点击加入](https://t.me/MSC4652)】

---

### 🧰 自用工具 & 资源库

* **📦 个人资源站 (综合导航)**：【👉 [点此访问](https://nuannuan-tools.vercel.app/)】
* **☁️ PikPak 磁力下载**：【⚡️ [点击注册](https://mypikpak.com/drive/activity/invited?invitation-code=66396543)】
* **📂 123 网盘资源**：【📂 [点击查看](https://www.123684.com/s/R2hjVv-6Pg13) 】*(提取码: NUAN)*
* **🔧 自用内网穿透工具**：【👉 [立即试用](https://www.cpolar.com/?channel=0&invite=6DaX)】

### 🛒 流量 & 账号服务

* **📶 正规运营商流量卡**：【📶 [点击下单](https://bankala.cn/s/f25188b9)】
* **💳 Bitget Wallet (免费外币卡)**：【👉 [点击申请](https://web3.bitget.com/share/1gWULE?inviteCode=NUAN1145)】
* **🔶 币安 (Binance) 交易所**：【💰 [注册领新手礼](https://www.bmwweb.academy/referral/earn-together/refer2earn-usdc/claim?hl=zh-TC&ref=GRO_28502_RAE9X&utm_source=default)】
* **🍎 Apple ID / 充值卡 / 代理**：【🍎 [购买链接](https://goso002.com/?from=24529)】
* **🌍 海外账号 / AI / 流媒体 / 游戏**：【🛍️ [购买链接](https://accboyytbnn.acceboy.com/)】

### 🚀 网络加速

* **🌐 自用机场推荐**：【🌐 [查看详情](https://t.me/NUAN114514/5)】

---

> ⚠️ **提示**：部分资源链接可能随时间失效，请以我的 TG 频道最新动态为准。
